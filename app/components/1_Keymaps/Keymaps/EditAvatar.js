...
class EditKeymapModal extends Component {
  ...
  handleChangeFilter(event) {
    ...
    this.setState({
      icons: fontAwesomeIcons.filter(iconName => iconName.indexOf(filter) !== -1)
    });
  }
  ...
  render() {
    ...
    const availableIcons = icons.map((icon, i) => {
      if (i < 250) {
        return <Avatar
                  onTouchTap={() => this.props.handleChangeIcon('fa '+icon)}
                  key={`avatar-${icon}`}
                  style={{
                    cursor: 'pointer',
                    margin: '0.5em'}}
    ...
    return (
      <div>
        ...
        <Avatar
          icon={<FontIcon className={this.props.iconClass} />}
          style={{
            top: '1.3em',
            right: '1.3em',
            position: 'absolute',
          ...
          onTouchTap={this.props.requestOpen}
        />

        <Dialog
          title="I heard you like icons ? 😉"
          contentStyle={{
            width: '90%',
            maxWidth: '1200px'
          }}
        ...
        >
          ...
          >
            ...
            <div style={{height: '500px', maxHeight: 'auto'}}>
              {availableIcons}
            </div>
